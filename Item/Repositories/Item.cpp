...
{
    Item::Item(std::shared_ptr<service::Database> database) : Database(std::move(database)) {}
    ...
    bool Item::Create(const dto::CreateItem &item) const
    {
        try
        {
            std::string sQuery = "INSERT INTO Items ("
                                 "[ItemEntry], "
                                 "[Code], "
                                 "[Name], "
                                 "Description, "
    ...
    int Item::GetCurrentSequence() const
    {
        try
        {
            const std::string sQuery = "SELECT ISNULL(ItemSequence, 0) + 1 ItemSequence FROM Sequences WHERE SeqEntry = 1";
    ...
    bool Item::UpdateUserSequence() const
    {
        try
        {
            const std::string sQuery = "UPDATE Sequences SET ItemSequence = ISNULL(ItemSequence,0) + 1";
