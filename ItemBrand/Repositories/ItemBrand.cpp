...
{
    ItemBrand::ItemBrand(std::shared_ptr<service::Database> _database) : database(std::move(_database)) {}

    int ItemBrand::GetCurrentSequence() const
    {
        try
        {
            const std::string sQuery = "SELECT ISNULL(ItemBrandsSequence, 0) + 1 ItemBrandsSequence FROM Sequences WHERE SeqEntry = 1";
    ...
    bool ItemBrand::UpdateItemBrandsSequence() const
    {
        try
        {
            const std::string sQuery = "UPDATE Sequences SET ItemBrandsSequence = ISNULL(ItemBrandsSequence,0) + 1";
        ...
        {
            throw std::runtime_error(std::string("[ItemBrandsSequence Exception] ") + " " + e.what());
        }
    }
    ...
    bool ItemBrand::Create(const dto::CreateItemBrand &createItemBrand) const
    {
        try
        {
            std::string sQuery = "INSERT INTO ItemBrands (ItBEntry, Code, Name, CreatedBy, CreateDate) VALUES (?, ?, ?, ?, ?);";

            std::vector<type::SQLParam> params = {
                GetCurrentSequence(),
                createItemBrand.Code,
                createItemBrand.Name,
                createItemBrand.CreatedBy,
                createItemBrand.CreateDate
            };
        ...
        catch (const std::exception &e)
        {
            ...
            throw(std::runtime_error(std::string("[CreateItemBrand Exception]") + e.what()));
        }
    }
    ...
    bool ItemBrand::Update(const dto::UpdateItemBrand &updateItemBrand) const
    {
       try
        {
            std::string sQuery = "UPDATE ItemBrands SET Name = ?, LastUpdatedBy = ?, UpdateDate = ? WHERE Code = ?;";

            std::vector<type::SQLParam> params = {
                updateItemBrand.Name.value(),
                updateItemBrand.LastUpdatedBy,
                updateItemBrand.UpdateDate,
                updateItemBrand.Code
            };
        ...
        {
            database->RollbackTransaction();
            throw(std::runtime_error(std::string("[UpdateItemBrand Exception]") + e.what()));
        }
    }
    ...
    type::Datatable ItemBrand::ReadAll() const
    {
        ...
        {
            throw std::runtime_error(std::string("[ReadAllItemBrands Exception] ") + " " + e.what());
        }
    }
    ...
    type::Datatable ItemBrand::Read(const dto::GetItemBrand itemBrand) const
    {
        ...
        {
            throw std::runtime_error(std::string("[ReadItemBrand Exception] ") + " " + e.what());
        }
    }
}