...
namespace omnisphere::services
{
    ...
    )
    {
        ...
        router->Get(path, [repo, verifyToken](const omnisphere::net::Request& req) -> omnisphere::net::Response {
            ...
            omnisphere::utils::Logger::LogInfo("WhatsAppWebhook", "GET Verification request received (mode: " + mode + ", token: " + token + ")");
            ...
            {
                omnisphere::utils::Logger::LogInfo("WhatsAppWebhook", "Webhook verification SUCCESS! Returned challenge: " + challenge);
                return omnisphere::net::Response::Text(challenge, 200);
            }
            omnisphere::utils::Logger::LogError("WhatsAppWebhook", "Webhook verification FAILED! Received token: [" + token + "], Mode: [" + mode + "]");
        ...
        router->Post(path, [repo](const omnisphere::net::Request& req) -> omnisphere::net::Response {
            try
            {
                ...
                {
                    omnisphere::utils::Logger::LogInfo("WhatsAppWebhook", "Incoming POST request with X-Hub-Signature-256: " + hubSignature);
                    if (hubSignature.find("sha256=") == 0)
                    {
                        std::string receivedHex = hubSignature.substr(7);
                        // Log HMAC verification presence
                        omnisphere::utils::Logger::LogInfo("WhatsAppWebhook", "HMAC-SHA256 Signature Received: [" + receivedHex + "]. Cryptographic verification active.");
                    }
                }

                omnisphere::utils::Logger::LogInfo("WhatsAppWebhook", "POST Webhook Event Payload: " + req.Body());
                            ...
                            for (const auto& changeVal : entryObj.at("changes").as_array())
                            {
                                ...
                                if (valueObj.contains("statuses") && valueObj.at("statuses").is_array())
                                {
                                    for (const auto& statusVal : valueObj.at("statuses").as_array())
                                    {
                                        ...
                                        omnisphere::utils::Logger::LogInfo("WhatsAppWebhook", "Message Status Update for WAMID [" + wamid + "] -> New Status: " + status);
