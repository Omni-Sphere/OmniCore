...
namespace omnicore::repository
{
    ...
    int Session::GetCurrentSequence() const
    {
        try
        {
            const std::string sQuery = "SELECT ISNULL(SessionSequence, 0) + 1 SessionSequence FROM Sequences WHERE SeqEntry = 1";
    ...
    bool Session::UpdateSessionSequence() const
    {
        try
        {
            const std::string sQuery = "UPDATE Sequences SET SessionSequence = ISNULL(SessionSequence,0) + 1";
        ...
        {
            throw std::runtime_error(std::string("[SessionSequence Exception] ") + " " + e.what());
        }
    };
    ...
    type::Datatable Session::Read(const dto::Login &login) const
    {
        try
        {
            ...
            if (login.Code.has_value())
                sQuery += "T0.UserCode = T1.[Code] WHERE T1.[Code] = '" + login.Code.value() + "' ";

            if (login.Email.has_value())
                sQuery += "T0.UserEmail = T1.Email WHERE T1.Email = '" + login.Email.value() + "' ";

            if (login.Phone.has_value())
                sQuery += "T0.UserPhone = T1.Phone WHRE T1.Phone = '" + login.Phone.value() + "' ";

            sQuery += "AND T0.DeviceIP = '" + login.DeviceIP + "' AND T0.HostName = '" + login.HostName + "' AND T0.IsActive = 'Y'";
        ...
        {
            throw std::runtime_error(std::string("[ReadSessionData Exception] ") + " " + e.what());
        }
    };
    ...
    type::Datatable Session::Read(const std::string &sessionUUID) const
    {
        try
        {
            std::string sQuery = "SELECT SessionUUID, StartDate, EndDate, DurationSeconds, Reason, LogoutMessage FROM Sessions WHERE SessionUUID = ?";
    ...
    type::Datatable Session::ExistsUUID(const std::string &sessionUUID) const
    {
        try
        {
            std::string sQuery = "SELECT COUNT(*) Total FROM Sessions WHERE SessionUUID  = ?";
        ...
        {
            throw std::runtime_error(std::string("[ValidateSessionUUID Exception] ") + " " + e.what());
        }
    };
    ...
    type::Datatable Session::IsActive(const std::string &sessionUUID) const
    {
        ...
        {
            throw std::runtime_error(std::string("[IsSessionActive Exception] ") + " " + e.what());
        }
    };
    ...
    bool Session::Close(const dto::Logout &logout) const
    {
        try
        {
            std::string sQuery = "UPDATE Sessions SET IsActive = 'N', EndDate = ?, DurationSeconds = ? ";
            std::vector<type::SQLParam> vParams;
        ...
        {
            throw std::runtime_error(std::string("[CloseSession Exception] ") + " " + e.what());
        }
    };
}