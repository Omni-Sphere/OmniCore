...
namespace omnicore::service
{
    ...
    model::AuthPayload Session::Login(const dto::Login &login) const
    {
        try
        {
            if (login.Code.has_value() && !user->Exists(enums::UserFilter::Code, login.Code.value()))
                throw std::runtime_error("User Code doesn't exists");

            if (login.Email.has_value() && !user->Exists(enums::UserFilter::Email, login.Email.value()))
                throw std::runtime_error("User Email doesn't exists");

            if (login.Phone.has_value() && !user->Exists(enums::UserFilter::Phone, login.Phone.value()))
                throw std::runtime_error("User Phone doesn't exists");

            if (login.Code.has_value() && !user->CheckPassword(enums::UserFilter::Code, login.Code.value(), login.Password))
                throw std::runtime_error("Wrong password");

            if (login.Email.has_value() && !user->CheckPassword(enums::UserFilter::Email, login.Email.value(), login.Password))
                throw std::runtime_error("Wrong password");

            if (login.Phone.has_value() && !user->CheckPassword(enums::UserFilter::Phone, login.Phone.value(), login.Password))
                throw std::runtime_error("Wrong password");
            ...
            std::cout << "Session service: " << std::string(data[0]["SessionUUID"]);
            ...
            if (login.Code.has_value())
                authPayload.User = std::make_shared<model::User>(user->Get(enums::UserFilter::Code, data[0]["UserCode"]));

            if (login.Email.has_value())
                authPayload.User = std::make_shared<model::User>(user->Get(enums::UserFilter::Email, data[0]["UserEmail"]));

            if (login.Phone.has_value())
                authPayload.User = std::make_shared<model::User>(user->Get(enums::UserFilter::Phone, data[0]["UserPhone"]));
    ...
    bool Session::Active(const std::string &sessionUUID) const
    {
        ...
        {
            throw std::runtime_error(std::string("[ActiveSession Exception] ") + e.what());
        }
    }
    ...
    bool Session::Exists(const std::string &sessionUUID) const
    {
        try
        {
            ...
            if (sessionCount > 1)
                throw std::runtime_error(std::string("Inconsistences in sesion UUID, found more than one session"));

            return true;
        }
        catch (const std::exception &e)
        {
            throw std::runtime_error(std::string("[ExistsSession Exception] ") + e.what());
        }
    }
