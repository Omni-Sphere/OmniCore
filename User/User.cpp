  ...
  try {
    ...
    if (user->Update(uUser)) {
      ...
      model::User user(
          ...
          data[0]["Phone"].AsDBNull<std::string>(), data[0]["EmployeeEntry"],
...
bool User::ModifyPassword(const dto::ChangePassword &userDTO) const {
  try {
    ...
    if (userDTO.Code.has_value() &&
        !user->UpdatePassword(enums::UserFilter::Code, userDTO.Code.value(),
    ...
    if (data.RowsCount() > 0)
      for (int i = 0; i < data.RowsCount(); i++)
        users.emplace_back(
            ...
            data[i]["EmployeeEntry"].AsDBNull<int>(), data[i]["SuperUser"],
