...
{
    User::User(std::shared_ptr<service::Database> _database) : database(std::move(_database)) {}
    ...
    bool User::Create(const dto::CreateUser &user) const
    {
        try
        {
            ...
            std::cout << "Password text : " << "[" << user.Password << "]\n";
    ...
    bool User::UpdateUserSequence() const
    {
        try
        {
            const std::string sQuery = "UPDATE Sequences SET UserSequence = ISNULL(UserSequence,0) + 1";
    ...
    int User::GetCurrentSequence() const
    {
        try
        {
            const std::string sQuery = "SELECT ISNULL(UserSequence, 0) + 1 UserSequence FROM Sequences WHERE SeqEntry = 1";
    ...
    bool User::UpdatePassword(const enums::UserFilter &filter, const std::string &value, const std::string &oldPassword, const std::string &newPassword) const
    {
        try
        {
            std::string sQuery = "UPDATE Users SET Password = ? WHERE ";

            const std::vector<uint8_t> hashedPassword = Hasher::HashPassword(newPassword);
        ...
        {
            database->RollbackTransaction();
            throw std::runtime_error(std::string("[UpdatePassword Exception]: ") + e.what());
        }
    };

    type::Datatable User::Read(const enums::UserFilter &filter, const std::string &value) const
    {
        try
        {
    ...
    bool User::ValidatePassword(const enums::UserFilter &searchFilter, const std::string &filterValue, const std::string &Password) const
    {
        try
        {
            ...
            std::cout << "[Debug] ValidatePassword Query: " << sQuery << "\n";
        ...
        {
            database->RollbackTransaction();
            throw std::runtime_error(std::string("[ValidatePassword Exception]: ") + e.what());
        }
    };
    ...
    bool User::ExistsEntry(const int &entry) const
    {
        try
        {
            ...
            type::Datatable data = database->FetchPrepared(sQuery, std::to_string(entry));
    ...
    bool User::ExistsCode(const std::string &code) const
    {
        try
        {
            const std::string sQuery = "SELECT ISNULL(COUNT(*), 0) Total FROM Users WHERE [Code] = ?";
